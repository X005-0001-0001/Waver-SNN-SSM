...
# ============================================================
# 210002proplus_maxpppp2_elite.py — 实值化精英版 — 流式 JSONL 数据加载版本
...
#           中间不再出现 complex 张量 → 可安全开 bf16 AMP；参数空间与 180 ultra 完全一致（可直接对照/迁移权重）。
# 对应源文件：180002proplus_maxpppp2_ultra.py（复值母本）
...
#   K=2 (STATE_DIM=4)  ← 默认（与 180 ultra 严格同构）
    ...
    def _tokenize_conversation(self, convs):
        ...
        while i >= 1:
            ...
            if assistant_turn.get('role') == 'assistant' and user_turn.get('role') == 'user':
                ...
                pair_tokens = 0
                for turn in pair:
                    pair_tokens += len(self._encode(turn.get('content', ''))) + 2
