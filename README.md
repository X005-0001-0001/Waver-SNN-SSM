好的，我来为你写一个详细的 README，整合你提供的所有训练信息、数据集规模和架构细节。
...
> 📌 **解读**：当前 Loss 稳定在 6.0 附近，PPL 约 407，表明模型正在有效学习。各层 `wave_raw_std` 从 L0 到 L4 逐渐收敛（L1 的 166 为初期瞬态，后续层已稳定在 3~5 左右），说明信号传播正常。MoE 专家权重范数（W_gate/W_up/W_down）集中在 1.0~1.1 之间，路由负载均衡良好，未出现专家坍缩。
