...
class ParallelComplexWaveLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor,
        ...
        A_disc[..., 0::2, 1::2] = -thd
        A_disc[..., 1::2, 0::2] = thd
        A_disc = torch.linalg.matrix_exp(A_disc * delta.unsqueeze(-1).unsqueeze(-1))   # exp(δ·Ω_t) (B,T,D,S,S)
    ...
    def step(self, x_t, state):
        ...
        Omega[:, 0::2, 1::2] = -thd
        Omega[:, 1::2, 0::2] = thd
        A_disc = torch.linalg.matrix_exp(Omega * delta.view(D, 1, 1))       # (D,S,S) = exp(δ·Ω_t)
    ...
    def forward(self, input_ids: torch.Tensor, labels: Optional[torch.Tensor] = None,
        ...
        for i, layer in enumerate(self.layers):
            if USE_GRADIENT_CHECKPOINT and self.training and labels is not None:
                ...
                output_wave = layer(layer_input, time_mask=time_mask)
            if self.use_residual:
