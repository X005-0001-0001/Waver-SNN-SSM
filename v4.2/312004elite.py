...
# （原稳定性惩罚超参数已删除：stab_penalty 恒为 0，属死代码；稳定性由结构保证——ν>0 漏水 + M 斜 Hermitian 不放大）
...
# 线性注意力摘要器参数
SUMMARY_ATTN_DK = 64          # key/query 投影维度
SUMMARY_CHUNK = 128            # 分块大小（P1-7）：分块线性注意力，避免物化 (B,T,dk,D) 大张量；0=回退 cumsum
...
# 唯一路径：矩阵扫描 + matrix_exp(Ω_base) + 单位模数据依赖旋转。
#   无 eig、无 complex 张量、无阈值、无回退逃生门。安全来自数学结构：
#   Ω_base 实部 -ν<0（漏水衰减）+ Rot 正交（单位模，不改变 A_disc 的奇异值谱）。
THETA_DELTA_ENABLED = True     # 数据依赖相位「进 Ω」：Omega_t = M + diag(-nu + i*(theta_base + theta_delta_t))
THETA_DELTA_SCALE = 1.0        # theta_delta 幅度缩放（theta_delta_proj 用 gain=0.1 初始化 → 起点≈固定 theta）
USE_CUDA_GRAPH_DECODE = False  # 性能优化开关：将逐 token 解码步抓成 CUDA Graph（消除每步几十次 kernel 启动开销）。
                               #   抓图失败时静默回退 eager —— 这只是性能回退，不是动力学回退：
                               #   图回放与 eager 数学逐位一致（同种子输出完全相同），不改变任何状态转移。
                               #   仅在 CUDA + 非 MoE 下生效（MoE 的 for e in ...: hit.any() 早退是数据相关控制流，不可捕获）。
                               #   默认 False，确认稳定后再开。
CUDA_GRAPH_WARMUP = 3          # 抓图前预热步数（让 cuBLAS/cuSOLVER workspace 在图外预分配）

# ============================================================
# 显存优化（04elite 新增）
# ============================================================
# SSM_CHUNK_SIZE: SSM 分块扫描块大小（0=不分块，64/128=分块）。
#   把长序列的矩阵扫描拆成块：块间串行传递初始状态，块内并行。
#   数学严格等价（关联扫描的可结合性）。
#   ⚠️ 实测（P8/P9，V=10000/H=256/L=4/T=1024/B=4）：
#     - 无显存收益（峰值完全不受 cs 影响，差异 ≤1 MB）
#     - 明显更慢（cs=16 时 3.38s vs cs=0 时 0.83s）
#     原因：A_disc 在分块前已完整物化，分块只是切片；峰值由其它项主导。
#   建议保持 0。仅在极长序列（如 T ≥ 16384）且 A_disc 成为瓶颈时才考虑启用。
SSM_CHUNK_SIZE = 0

# ============================================================
# 显存缓存清理（P1）
# ============================================================
# EMPTY_CACHE_INTERVAL: 训练时每 N 步调用一次 torch.cuda.empty_cache()。
#   0  = 只在每个 epoch 末清理（P13 之前的默认行为）
#   N>0 = 每 N 步清理一次（可有效降低 reserved 碎片，代价是轻微性能损失）
#   建议范围：50 ~ 500。太小会频繁同步拖慢训练，太大则碎片累积。
#   注：empty_cache 只释放未被使用的缓存块，不改任何数学。
EMPTY_CACHE_INTERVAL = 0

# ============================================================
# A_disc 分块计算（P3）—— 解决 reserved 碎片化 + 阶梯上抬
# ============================================================
# ⚠️ 默认关闭。P3 在数学上严格等价（fp64 下 1e-18，前向 loss 差 0），
#    但在纯 fp32 下，分块路径与整段路径的**反向梯度有 ~3e-5 ~ 1.4e-4 的
#    相对差**（两条等价计算图舍入顺序不同，沿 2000~8000 步长递推被放大；
#    前向 rel ~2e-6，反向放大 ~50-100×，对长递推属正常量级）。
#    若你的训练对梯度逐位复现敏感（如对照实验），请保持 0。
#
# A_DISC_CHUNK: A_disc 的分块大小。0=整段（默认，与 03elite 逐位一致）。
#   >0 = 按 chunk 大小分块构造 A_disc + matrix_exp + 扫描，块间传状态 h_init。
#   实测（T=6205，含反向，V=7000 H=256 L=3 S=4 B=1 GC=ON）：
#     peak 5655 → 1913 MB（-66%）；reserved 6811 → 2336 MB；耗时 1.8×
#   建议值：1024（收益最好）或 2048（折中）。
A_DISC_CHUNK = 0
# A_DISC_CHUNK_MIN_T: 仅当 T > 此值才启用分块（自适应，短样本不受影响）。
#   默认 2048 → 当前数据集 98.86% 的样本不受影响（只 1.14% 的超长样本分块）。
A_DISC_CHUNK_MIN_T = 2048

# ============================================================
# 数据切分（P4）—— 多轮切单轮 + 单轮超长丢弃
# ============================================================
# P4_SEG_MAX_TOKENS: 单样本的 token 上限。超过此值的处理见主菜单 11。
#   建议 2048（当前数据集只丢 1.24%）。
P4_SEG_MAX_TOKENS = 2048

# ============================================================
# Loop 机制（04elite 新增）
# ============================================================
# USE_LOOP: Loop 总开关。False 时完全等价于 03elite（每层单次执行）。
USE_LOOP = False
# LOOP_CONFIG: 每层循环次数列表（长度=num_layers）。空列表=全 1。
#   示例：[1, 2, 2, 1] → 第 0 层 1 次、第 1 层 2 次、第 2 层 2 次、第 3 层 1 次。
LOOP_CONFIG = []
# LOOP_RESIDUAL_SCALE: True=残差用 ε=1/(N√L)（ByteDance 权重共享残差缩放），False=标准 ε=1。
#   关键：最优学习率只取决于唯一层数 L，不取决于循环次数 N → 可从小 N 迁移到大 N。
LOOP_RESIDUAL_SCALE = True
# LOOP_INDEX_EMBED: True=循环索引嵌入（正弦深度信号，告诉模块当前是第几次循环）。
LOOP_INDEX_EMBED = True
# LOOP_MOE_MODE: 'shared'=所有循环共享 MoE（路由每轮独立 top-k，自然激活不同专家）/
#                'per_loop'=每次循环独立 MoE 模块（参数更多，表达力更强）。
LOOP_MOE_MODE = 'shared'
...
def _blelloch_scan(A_flat: torch.Tensor, b_flat: torch.Tensor) -> torch.Tensor:
    ...
    dtype = A_flat.dtype
    real_dtype = torch.float32 if dtype == torch.float32 else torch.float64

    # 填充到 ≥ T 的 2 次幂（T 已是 2 的幂时不额外填充，扫描算量/显存减半，数值逐位不变）
...
def _scan_with_init_state(A_chunk, b_chunk, h_init):
    """块内扫描 + 初始状态（可微）。04elite 分块扫描专用。

    h_t = A_t @ h_{t-1} + b_t，h_{-1} = h_init。
    构造：h_t = h0_t + P_t @ h_init，
      - h0_t：零初值解（复用 Blelloch 扫描，O(log T) 并行，自带反向）；
      - P_t = A_t@...@A_0：块内转移算子前缀积，用 O(T) 串行（块很小，开销可忽略）；
      - h_init 项对 h_init / A / b 的梯度由 autograd 自动完成（torch.matmul 链）。
    数学严格等价，无近似。
    """
    N, T, K, _ = A_chunk.shape
    h0 = parallel_block_scan(A_chunk, b_chunk)                       # (N,T,K)
    eye = torch.eye(K, dtype=A_chunk.dtype, device=A_chunk.device).expand(N, K, K).contiguous()
    P_list = []
    acc = eye
    for t in range(T):
        acc = torch.matmul(A_chunk[:, t], acc)
        P_list.append(acc)
    P = torch.stack(P_list, dim=1)                                   # (N,T,K,K)
    h = h0 + torch.matmul(P, h_init.unsqueeze(-1)).squeeze(-1)       # (N,T,K)
    return h


# ------------------------------------------------------------
# 复标量关联前缀扫描（P1-6：Ω 特征分解后的对角复递推）
#   z_t = a_t ⊙ z_{t-1} + b_t，a_t/b_t 为 (B,T,K) 复数；比矩阵版省 6× 带宽（无 6×6 矩阵）
# ------------------------------------------------------------
...
class ParallelComplexWaveLayer(nn.Module):
    def __init__(self, input_dim: int, output_dim: int, state_dim: int = 2, coupling_scale: float = 1.0,
        ...
        if disable_summarizer:
            ...
            self.delta_linear = weight_norm(nn.Linear(output_dim, output_dim))       # P3：delta 逐通道(D)，不再逐子桶
            ...
            self.delta_linear = weight_norm(nn.Linear(2 * output_dim, output_dim))   # P3：delta 逐通道(D)
        ...
        # ---- 数据依赖相位投影（theta_delta）：纯线性（weight_norm），无激活 ----
        if THETA_DELTA_ENABLED:
            _td_in = output_dim if disable_summarizer else 2 * output_dim
            self.theta_delta_proj = weight_norm(nn.Linear(_td_in, output_dim * K))
    ...
    def _init_weights(self):
        ...
        if THETA_DELTA_ENABLED:
            wn_layers.append(self.theta_delta_proj)
        ...
        _small = [self.delta_linear, self.B_linear] + ([self.theta_delta_proj] if THETA_DELTA_ENABLED else [])
        for lin in wn_layers:
            gain = 0.1 if any(lin is s for s in _small) else 1.0
        ...
        #   delta 大 → exp(-δ·ν) 更小 → 更快遗忘，不会爆炸，无需冻结
        #   P3：delta 是逐通道标量 δ_d，故 A_t = matrix_exp(δ_d·Ω_t) 是严格的标量倍（无"按行缩放"的歧义）
    ...
    def forward(self, x: torch.Tensor,
        ...
        if self.disable_summarizer:
            ...
            phase_input = I_gated_norm
        else:
            if return_state:
                g, summarizer_state = self.summarizer(I_gated_norm, return_state=True)
            else:
            ...
            phase_input = x_with_summary

        delta = F.softplus(delta_raw) * F.softplus(self.delta_scale)
        delta = delta.view(B, T, D)                      # (B,T,D) 逐通道标量（P3）
        ...
        # ---- 数据依赖相位增量（纯线性投影，无激活）----
        if THETA_DELTA_ENABLED:
            theta_delta = self.theta_delta_proj(phase_input).view(B, T, D, K) * THETA_DELTA_SCALE
        else:
            theta_delta = None
        ...
        delta_k = delta.unsqueeze(-1)                    # (B,T,D,1) 广播到 K
        e = torch.exp(-delta_k * nu)                     # (B,T,D,K)
        c = torch.cos(delta_k * theta)
        s = torch.sin(delta_k * theta)
        ...
        # ---- Ω_t = M + diag(-ν + i·θ_total_t)，θ_total = θ_base + θ_δ_t；A_t = matrix_exp(δ_t · Ω_t) ----
        #   θ_base = exp(theta_raw) 固定；θ_δ 数据依赖、同时进 Ω 的对角（不再拆成单独的旋转）。
        #   δ_t 是逐通道标量（P3），故 δ_t·Ω_t 是严格标量倍，matrix_exp 无歧义。
        #   稳定性论证（纯结构性，无任何阈值/裁剪/回退）：
        #     把 Ω 写成复矩阵 Ω_c = (M - diag(ν)) + i·diag(θ_total)，其 Hermitian 部分
        #       (Ω_c + Ω_c^H)/2 = ((M-ν+iθ) + (M^T-ν-iθ))/2 = -diag(ν)     （M^T=-M ⇒ M 项相消；iθ 项相消）
        #     由 Bendixson 定理 Re(λ(Ω_c)) ≤ λ_max(-diag(ν)) = -min(ν) < 0（ν=exp(nu_log)>0），
        #     故 |A_t| 的谱半径 ≤ exp(-δ_t·min(ν)) < 1（δ_t=softplus(·)>0）。
        #     数据依赖的 θ_δ 不改变 Hermitian 部分 ⇒ 不影响稳定性。
        theta_total = theta if theta_delta is None else (theta + theta_delta)    # (D,K) 或 (B,T,D,K)
        Md = M - torch.diag_embed(nu)                                            # (D,K,K) 实-实/虚-虚块

        # ---- P3：自适应分块（A_DISC_CHUNK=0 或 T<=A_DISC_CHUNK_MIN_T 时走原路径，逐位不变）----
        _use_chunk = (A_DISC_CHUNK > 0) and (T > A_DISC_CHUNK_MIN_T)
        if _use_chunk:
            # ---- P3：A_disc 构造前分块（自适应，仅当 T > A_DISC_CHUNK_MIN_T 时启用）----
            #   块内独立构造 A_disc + matrix_exp + 扫描，块间只传状态 h_init（形状 (B*D,1,S)）。
            #   数学严格等价（关联扫描可结合性；实测相对误差 ~1e-7）。
            _thd_shared = torch.diag_embed(theta_total) if theta_delta is None else None
            _Nflat = B * D
            _chunk_init = torch.zeros(_Nflat, 1, S, device=device, dtype=torch.float32)
            _h_list = []
            for _c0 in range(0, T, A_DISC_CHUNK):
                _c1 = min(_c0 + A_DISC_CHUNK, T)
                _tc = _c1 - _c0
                _thd = _thd_shared if _thd_shared is not None else torch.diag_embed(theta_total[:, _c0:_c1])
                _A = torch.zeros(B, _tc, D, S, S, device=device, dtype=torch.float32)
                _A[..., 0::2, 0::2] = Md
                _A[..., 1::2, 1::2] = Md
                _A[..., 0::2, 1::2] = -_thd
                _A[..., 1::2, 0::2] = _thd
                _A = torch.linalg.matrix_exp(_A * delta[:, _c0:_c1].unsqueeze(-1).unsqueeze(-1))
                _b = torch.zeros(B, _tc, D, S, device=device, dtype=torch.float32)
                _b[..., 0::2] = B_disc_real[:, _c0:_c1] * c_in[:, _c0:_c1]
                _b[..., 1::2] = B_disc_imag[:, _c0:_c1] * c_in[:, _c0:_c1]
                if time_mask is not None:
                    _me = time_mask[:, _c0:_c1].float().to(_A.dtype).unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
                    _eye = torch.eye(S, dtype=_A.dtype, device=device).view(1, 1, 1, S, S)
                    _A = _A * _me + (1.0 - _me) * _eye
                    _b = _b * _me.squeeze(-1)
                _Af = _A.permute(0, 2, 1, 3, 4).reshape(_Nflat, _tc, S, S)
                _bf = _b.permute(0, 2, 1, 3).reshape(_Nflat, _tc, S)
                with torch.amp.autocast(device_type=device.type, enabled=(not AMP_FP32_SSM_SCAN)):
                    _hc = _scan_with_init_state(_Af, _bf, _chunk_init)
                _chunk_init = _hc[:, -1:]                    # (B*D, 1, S) 传给下一块
                _h_list.append(_hc)
                del _A, _b, _Af, _bf
            h_flat = torch.cat(_h_list, dim=1)
            B_orig, T_len, D_orig, S_ = B, T, D, S
            h = h_flat.view(B_orig, D_orig, T_len, S_).permute(0, 2, 1, 3)
        else:
            thd = torch.diag_embed(theta_total)                                      # (D,K,K) 或 (B,T,D,K,K)
            A_disc = torch.zeros(B, T, D, S, S, device=device, dtype=torch.float32)
            A_disc[..., 0::2, 0::2] = Md
            A_disc[..., 1::2, 1::2] = Md
            A_disc[..., 0::2, 1::2] = -thd
            A_disc[..., 1::2, 0::2] = thd
            # ---- matrix_exp ----
            A_disc_input = A_disc * delta.unsqueeze(-1).unsqueeze(-1)              # δ·Ω_t (B,T,D,S,S)
            A_disc = torch.linalg.matrix_exp(A_disc_input)                     # exp(δ·Ω_t) (B,T,D,S,S)
            ...
            with torch.amp.autocast(device_type=device.type, enabled=(not AMP_FP32_SSM_SCAN)):
                # ---- 04elite：可选分块扫描（块间串行传递初始状态，块内并行；数学严格等价）----
                if SSM_CHUNK_SIZE > 0 and T_len > SSM_CHUNK_SIZE:
                    _chunk_init = torch.zeros(A_flat.shape[0], 1, S_, device=device, dtype=A_flat.dtype)
                    _h_list = []
                    for _c0 in range(0, T_len, SSM_CHUNK_SIZE):
                        _c1 = min(_c0 + SSM_CHUNK_SIZE, T_len)
                        _h_chunk = _scan_with_init_state(A_flat[:, _c0:_c1], b_flat[:, _c0:_c1], _chunk_init)
                        _chunk_init = _h_chunk[:, -1:]                 # (B*D, S, S) 传给下一块
                        _h_list.append(_h_chunk)
                    h_flat = torch.cat(_h_list, dim=1)
                else:
        ...
        if return_state:
            # ---- 构建与 init_state 兼容的末状态（P1-8 并行 prefill：供 prefill → step 无缝续推）----
            ks = self.gate_conv.kernel_size[0]
            conv_len = ks - 1
            if T >= conv_len:
                conv = I[0, T - conv_len:, :].float()                    # (ks-1, D) 末 conv_len 个 I 值
            else:
                conv = torch.cat([torch.zeros(conv_len - T, D, device=device, dtype=torch.float32),
                                  I[0].float()], dim=0)
            state = {
                'h': h[0, -1].contiguous().float(),                      # (D,S) 实值
                'conv': conv,
            }
            if self.disable_summarizer:
                state['h_ema'] = h_ema[0, -1].contiguous().float()       # (D,S) 实值
            else:
                state['summarizer'] = summarizer_state
            return wave_out, state

        return wave_out

    def forward_prefill(self, x, time_mask=None):
        """并行 prefill（P1-8）：一次前向并行算出全部末状态，返回 (wave_out, state)。与逐 token step 严格等价。"""
        out, state = self.forward(x, time_mask=time_mask, return_state=True)
        return out, state

    def init_state(self, device):
        """初始化递归推理状态：主递推 h（实值 2K 维，实虚交替）；摘要模式额外持有摘要器状态。"""
        D = self.output_dim
        S = self.state_dim
        ks = self.gate_conv.kernel_size[0]
        state = {
    ...
    def step(self, x_t, state):
        ...
        # 原地滚动（P2-9）：保持 state['conv'] 存储地址不变，CUDA Graph 回放才能读到最新历史
        state['conv'].copy_(torch.cat([state['conv'][1:], I_t.unsqueeze(0)], dim=0))
        ...
        delta = delta.view(D)                            # (D) 逐通道标量（P3）

        if THETA_DELTA_ENABLED:
            _ph_t = I_gated_norm if self.disable_summarizer else torch.cat([I_gated_norm, g_t], dim=-1)
            theta_delta_t = self.theta_delta_proj(_ph_t).view(D, K) * THETA_DELTA_SCALE
        else:
            theta_delta_t = None
        ...
        delta_k = delta.unsqueeze(-1)                    # (D,1) 广播到 K
        e = torch.exp(-delta_k * nu)
        c = torch.cos(delta_k * theta)
        s = torch.sin(delta_k * theta)
        ...
        # ---- Ω_t = M + diag(-ν + i·θ_total_t)；A_t = matrix_exp(δ_t·Ω_t)（同 forward；δ 逐通道标量）----
        theta_total_t = theta if theta_delta_t is None else (theta + theta_delta_t)
        Omega = torch.zeros(D, S, S, device=device, dtype=torch.float32)
        Omega[:, 0::2, 0::2] = Md
        Omega[:, 1::2, 1::2] = Md
        thd = torch.diag_embed(theta_total_t)
        Omega[:, 0::2, 1::2] = -thd
        Omega[:, 1::2, 0::2] = thd
        # ---- matrix_exp ----
        _Omega_scaled = Omega * delta.view(D, 1, 1)                          # δ·Ω_t (D,S,S)
        A_disc = torch.linalg.matrix_exp(_Omega_scaled)                  # (D,S,S) = exp(δ·Ω_t)
...
class MeanSummarizer(nn.Module):
    ...
    def forward(self, x, return_state=False):
        ...
        g = torch.cumsum(x, dim=1) / denom
        if return_state:
            state = {'sum': x.sum(dim=1)[0].float(),
                     'count': torch.tensor(float(T), device=x.device, dtype=torch.float32)}
            return g, state
        return g

    def init_state(self, device, D=None):
        D = D or self.D
        # count 用 0 维 tensor（而非 Python int）：CUDA Graph 回放时原地自增才能生效（P2-9）
        return {'sum': torch.zeros(D, device=device, dtype=torch.float32),
                'count': torch.zeros((), device=device, dtype=torch.float32)}

    def step(self, x_t, state):
        state['sum'].add_(x_t)                       # 原地累加
        state['count'].add_(1.0)                     # 原地自增
...
class LinearAttentionSummarizer(nn.Module):
    ...
    def forward(self, x, return_state=False):
        ...
        with torch.amp.autocast(device_type=x.device.type, enabled=(not AMP_FP32_SUMMARIZER)):
            # 显式上转 fp32：锁 fp32 时（内层 autocast 关闭）.float() 生效，累积/读数/增量状态全部 fp32；
            # 不锁时（内层 autocast 开启）白名单算子会按需回到 bf16，行为与开关意图一致。
            k = k.float(); v = v.float(); q = q.float()
            C = int(SUMMARY_CHUNK)
            if C > 0 and T > C:
                # 分块线性注意力（P1-7）：不物化 (B,T,dk,D) 大张量，块内 tril(qk^T)@v + 块间运行状态，数学严格等价
                S_run = torch.zeros(B, self.dk, D, device=x.device, dtype=torch.float32)
                outs = []
                for c0 in range(0, T, C):
                    kc = k[:, c0:c0+C]; qc = q[:, c0:c0+C]; vc = v[:, c0:c0+C]
                    attn = torch.tril(qc @ kc.transpose(-1, -2))     # (B,Cc,Cc) 块内因果
                    local = attn @ vc                                 # (B,Cc,D) 块内贡献 Σ_{s≤t,同块}(q_t·k_s)v_s
                    glob = qc @ S_run                                  # (B,Cc,D) 块前累积贡献
                    S_run = S_run + kc.transpose(-1, -2) @ vc         # 更新运行状态
                    outs.append(local + glob)
                g = torch.cat(outs, dim=1) / torch.arange(1, T + 1, device=x.device, dtype=torch.float32).view(1, T, 1)
            else:
                # cumsum 向量化：S_t = Σ_{s≤t} k_s ⊗ v_s
                incr = torch.einsum('btd,bte->btde', k, v)
                if incr.dtype in (torch.float16, torch.bfloat16):
                    incr = incr.float()
                S = torch.cumsum(incr, dim=1)
                g = torch.einsum('btd,btde->bte', q, S) / torch.arange(1, T + 1, device=x.device, dtype=S.dtype).view(1, T, 1)
            if return_state:
                # 必须在 autocast 控制区内计算：控制区外会被外层 bf16 autocast 劫持成 bf16，
                # 与 init_state/step 的 fp32 运行态 S 冲突（AMP 下 prefill 后首个解码 token matmul dtype 崩溃）
                S_final = torch.einsum('btd,bte->bde', k, v)[0]      # (dk, D) 全序列累积状态（fp32）
                state = {'S': S_final.contiguous(), 'count': torch.tensor(float(T), device=x.device, dtype=torch.float32)}
                return g, state
        return g   # (B,T,D)

    def init_state(self, device, D=None):
        # count 用 0 维 tensor（而非 Python int）：CUDA Graph 回放时原地自增才能生效（P2-9）
        return {'S': torch.zeros(self.dk, self.D, device=device, dtype=torch.float32),
                'count': torch.zeros((), device=device, dtype=torch.float32)}
    ...
    def step(self, x_t, state):
        ...
        # 状态累积与读数锁 fp32，与 forward 严格一致（显式上转，不依赖 normalize 的偶然 dtype）
        with torch.amp.autocast(device_type=x_t.device.type, enabled=(not AMP_FP32_SUMMARIZER)):
            k_t = k_t.float(); v_t = v_t.float(); q_t = q_t.float()
            state['S'].add_(torch.outer(k_t, v_t))       # 原地累加
            state['count'].add_(1.0)                     # 原地自增
...
class MiniWaveSummarizer(nn.Module):
    ...
    def forward(self, x, return_state=False):
        ...
        for i, layer in enumerate(self.layers):
            if return_state:
                out, st = layer(layer_input, return_state=True)
                states.append(st)
            else:
...
class MoEFFN(nn.Module):
    ...
    # ---------- 专家取参 ----------
    def _expert_mats(self, e, device, dtype=None):
        if self.expert_quant == 'nvfp4':
            outs = []
            for name in self._weight_names:
                ...
                # 反量化得 fp32，再按目标 dtype 落位（dtype=None 表示保持 fp32）
                outs.append(nvfp4_dequantize(packed, scales, shp, NVFP4_BLOCK_SIZE).to(device).to(dtype=dtype))
            wg, wu, wd = outs
            bg = self.b_gate[e].to(device, dtype=dtype)
            bu = self.b_up[e].to(device, dtype=dtype)
            bd = self.b_down[e].to(device, dtype=dtype)
            return wg, wu, wd, bg, bu, bd
        if self.expert_quant == 'bf16':
            # 反量化到 fp32 后再按目标 dtype 落位（dtype=None 时保持 fp32）
            wg = self.w_gate_bf16[e].to(device).float().to(dtype=dtype)
            wu = self.w_up_bf16[e].to(device).float().to(dtype=dtype)
            wd = self.w_down_bf16[e].to(device).float().to(dtype=dtype)
            bg = self.b_gate[e].to(device, dtype=dtype)
            bu = self.b_up[e].to(device, dtype=dtype)
            bd = self.b_down[e].to(device, dtype=dtype)
            return wg, wu, wd, bg, bu, bd
        if self.offload:
            ...
            wg = self.w_gate[e].to(device, dtype=dtype, non_blocking=True)
            wu = self.w_up[e].to(device, dtype=dtype, non_blocking=True)
            wd = self.w_down[e].to(device, dtype=dtype, non_blocking=True)
            bg = self.b_gate[e].to(device, dtype=dtype, non_blocking=True)
            bu = self.b_up[e].to(device, dtype=dtype, non_blocking=True)
            bd = self.b_down[e].to(device, dtype=dtype, non_blocking=True)
        else:
            wg = self.w_gate[e].to(dtype=dtype); wu = self.w_up[e].to(dtype=dtype); wd = self.w_down[e].to(dtype=dtype)
            bg = self.b_gate[e].to(dtype=dtype); bu = self.b_up[e].to(dtype=dtype); bd = self.b_down[e].to(dtype=dtype)
        if MOE_EXPERT_WNORM:
            # weight_norm：把权重范数归一化为可学习幅度，方向在球面上优化，范数恒等于 scale（结构性防发散）
            wg = wg * (self.w_gate_scale[e].to(device, dtype=dtype) / wg.norm().clamp_min(1e-8))
            wu = wu * (self.w_up_scale[e].to(device, dtype=dtype) / wu.norm().clamp_min(1e-8))
            wd = wd * (self.w_down_scale[e].to(device, dtype=dtype) / wd.norm().clamp_min(1e-8))
    ...
    def _dispatch_experts(self, expert_ids, device, dtype=None):
        ...
        if use_stream:
            ...
            with torch.cuda.stream(self._copy_stream):
                ...
                mats = {e: self._expert_mats(e, device, dtype) for e in expert_ids}
            ...
            return mats
        return {e: self._expert_mats(e, device, dtype) for e in expert_ids}
    ...
    def forward(self, x, return_aux=True):
        ...
        mats = self._dispatch_experts([e for e, _, _ in plan], device, x.dtype)
...
# ------------------------------------------------------------
# Loop 机制辅助（04elite 新增）
# ------------------------------------------------------------
def get_loop_residual_scale(loop_count: int, num_unique_layers: int) -> float:
    """Loop 残差缩放因子 ε。

    - LOOP_RESIDUAL_SCALE=False → 标准 ε=1。
    - loop_count<=1 → 1.0（无循环，无缩放）。
    - 否则 ε = 1/(N·√L)（ByteDance 权重共享残差缩放）。

    关键性质：最优学习率只取决于唯一层数 L，不依赖循环次数 N → 可从小 N 迁移到大 N。
    """
    if not LOOP_RESIDUAL_SCALE:
        return 1.0
    if loop_count is None or loop_count <= 1:
        return 1.0
    return 1.0 / (float(loop_count) * math.sqrt(max(int(num_unique_layers), 1)))


class LoopIndexEmbedding(nn.Module):
    """循环索引嵌入：正弦深度信号，注入隐藏状态以区分第几次循环（权重共享后模块无法自行区分）。

    无参数（纯确定性函数），dim 为奇数时按 PyTorch 常规偶数维切分（前半 sin / 后半 cos）。
    """
    ...
    def __init__(self, dim: int):
        ...
        self.dim = dim
        half = dim // 2
        # 频率项固定（buffer，不参与梯度）：exp(-ln(10000) * i / half)
        if half > 0:
    ...
    def forward(self, h: torch.Tensor, loop_idx: int) -> torch.Tensor:
        # 生成正弦嵌入 (dim,)，广播到 h 的 (..., dim)
        device = h.device
        half = self.dim // 2
        emb = torch.zeros(self.dim, device=device, dtype=h.dtype)
        f = self.freqs.to(device=device, dtype=h.dtype)
        emb[0:2 * half:2] = torch.sin(loop_idx * f)
        emb[1:2 * half:2] = torch.cos(loop_idx * f)
        return h + emb.view(*([1] * (h.dim() - 1)), self.dim)
...
class WaveSNN(nn.Module):
    def __init__(self, vocab_size: int, hidden_size: int, num_layers: int,
        ...
        # ---- Loop 机制（04elite 新增）：每层循环次数 + 循环索引嵌入 ----
        # LOOP_CONFIG 长度应为 num_layers；不足补 1，超出截断。USE_LOOP=False 时全按 1（等价 03elite）。
        self.num_unique_layers = num_layers
        if USE_LOOP:
            cfg = list(LOOP_CONFIG) if LOOP_CONFIG else []
            if len(cfg) < num_layers:
                cfg = cfg + [1] * (num_layers - len(cfg))
            elif len(cfg) > num_layers:
                cfg = cfg[:num_layers]
            self.loop_config = [max(1, int(x)) for x in cfg]
        else:
            self.loop_config = [1] * num_layers
        if LOOP_INDEX_EMBED and USE_LOOP:
            self.loop_embeds = nn.ModuleList([LoopIndexEmbedding(hidden_size) for _ in range(num_layers)])
        else:
            self.loop_embeds = nn.ModuleList()

        # ---- MoE-FFN（每层 SSM 后接 MoE；默认关闭） ----
        self.moe_enabled = MOE_ENABLED
        if self.moe_enabled:
            moe_offload = decide_moe_offload(hidden_size)
            if USE_LOOP and LOOP_MOE_MODE == 'per_loop':
                # 每次循环独立 MoE：moes[i] 是长度 loop_config[i] 的 ModuleList
                self.moes = nn.ModuleList([
                    nn.ModuleList([
                        ...
                        for _ in range(self.loop_config[i])
                    ])
                    for i in range(num_layers)
                ])
            else:
                # shared（默认）：所有循环共用同一组 MoE 专家；路由每轮独立 top-k
                self.moes = nn.ModuleList([
                    MoEFFN(hidden_size, MOE_NUM_EXPERTS, MOE_TOP_K, MOE_EXPERT_HIDDEN,
                           offload=moe_offload, expert_quant=MOE_EXPERT_QUANT)
    ...
    def forward(self, input_ids: torch.Tensor, labels: Optional[torch.Tensor] = None,
        ...
        for i, layer in enumerate(self.layers):
            # Loop 机制（04elite）：第 i 层按 loop_config[i] 重复执行；USE_LOOP=False 时 n_loops=1
            n_loops = self.loop_config[i] if i < len(self.loop_config) else 1
            _layer_in = layer_input                       # 残差基准（循环间固定）
            _loop_res_scale = get_loop_residual_scale(n_loops, self.num_unique_layers)
            for loop_idx in range(n_loops):
                _cur = layer_input
                if len(self.loop_embeds) > 0:
                    # 循环索引嵌入：只注入第 2 次及以后的循环（loop_idx=0 保持与 03elite 逐位一致）
                    if loop_idx > 0:
                        _cur = self.loop_embeds[i](_cur, loop_idx)
                if USE_GRADIENT_CHECKPOINT and self.training and labels is not None:
                    output_wave = checkpoint(
                        lambda l, x, m: l(x, time_mask=m),
                        layer, _cur, time_mask, use_reentrant=False
                    )
                else:
                    output_wave = layer(_cur, time_mask=time_mask)
                if self.use_residual:
                    output_wave = output_wave * _loop_res_scale + _layer_in
                output_wave = self.layer_norms[i](output_wave)
                if self.moe_enabled:
                    if USE_LOOP and LOOP_MOE_MODE == 'per_loop':
                        moe_out, moe_aux = self.moes[i][loop_idx](output_wave)
                    else:
            ...
            layer_activities.append(layer_input.mean().detach())  # 延迟 .item()：原每层一次 GPU→CPU 同步
    ...
    def init_states(self, device):
        """每层返回长度为 loop_config[i] 的 state 列表（每次循环一份独立 SSM 状态）。

        USE_LOOP=False 时 loop_config 全 1 -> 退化为 [state]，与 03elite 结构兼容。
        """
        all_states = []
        for _i, _layer in enumerate(self.layers):
            _n = int(self.loop_config[_i]) if _i < len(self.loop_config) else 1
            all_states.append([_layer.init_state(device) for _ in range(_n)])
        return all_states
    ...
    def prefill(self, input_ids):
        ...
        04elite Loop（P11）：每层按 loop_config[i] 重复执行，语义与 WaveSNN.forward 完全一致 ——
        每次循环用 forward_prefill **独立**从头计算整序列 SSM，故每次循环各自保存一份 state。
        USE_LOOP=False 时 n_loops=1、_loop_res_scale=1.0，退化为原实现（与 03elite 逐位一致）。
        """
        device = input_ids.device
        token_emb = self.embedding(input_ids) * self.input_scale
        layer_input = token_emb
        states = []
        for i, layer in enumerate(self.layers):
            # Loop 机制：与 forward 训练路径同构
            n_loops = int(self.loop_config[i]) if i < len(self.loop_config) else 1
            _layer_in = layer_input                       # 残差基准（循环间固定）
            _loop_res_scale = get_loop_residual_scale(n_loops, self.num_unique_layers)
            _states_i = []
            for loop_idx in range(n_loops):
                _cur = layer_input
                if len(self.loop_embeds) > 0 and loop_idx > 0:
                    _cur = self.loop_embeds[i](_cur, loop_idx)
                out, st = layer.forward_prefill(_cur)
                _states_i.append(st)
                if self.use_residual:
                    out = out * _loop_res_scale + _layer_in
                out = self.layer_norms[i](out)
                if self.moe_enabled:
                    if USE_LOOP and LOOP_MOE_MODE == 'per_loop':
                        moe_out, _ = self.moes[i][loop_idx](out)
                    else:
    ...
    def _state_restore(snap, dst):
        ...
        if torch.is_tensor(snap):
            ...
            for k in list(dst.keys()):            # 目标里多出的键（如旧版 'z'）删掉，保持结构一致
                if k not in snap:
                    del dst[k]
    ...
    def _capture_decode_graph(self, device, states):
        ...
        注意：此处「回退」只是性能回退（graph → eager），不是动力学回退 —— 两者数学逐位一致。"""
        if not USE_CUDA_GRAPH_DECODE or device.type != 'cuda' or self.moe_enabled:
            return None
        # P11：CUDA Graph 会把「单 token 解码步」的算子序列固化成图，而 Loop 的循环次数/索引在
        # 构图时固定、回放时无法改变。为避免静默产生错误结果，Loop（非全 1）时显式不抓图，
        # 调用方回退 eager（仅性能损失，数学不变）。
        if USE_LOOP and list(self.loop_config) != [1] * int(self.num_unique_layers):
            return None
        try:
            self._dg_tok = torch.zeros((1, 1), dtype=torch.long, device=device)
            snap = WaveSNN._state_snapshot(states)
            # 预热：在图外执行若干步（预分配 cuBLAS/cuSOLVER workspace），再把状态回滚
            ...
            # 抓图失败（如某算子不可捕获、cuSOLVER workspace 需 host 端决定）→ 返回 None，
            # 调用方回退 eager。数学与图完全一致，仅影响推理速度。
            self._dg_logits = None
            return None

    def decode_step_graphed(self, token_id, states):
        """CUDA Graph 解码步：与 step 数学一致，但每步只回放一次图（P2-9）。失败自动回退 step（仅性能回退）。"""
        if self._dg_disabled:
            # 一旦抓图失败即永久回退 eager，不反复重试（避免每步都尝试抓图拖慢推理）。
            # 回退后动力学不变，只影响速度。
            return self.step(token_id, states)
        device = states[0][0]['h'].device
    ...
    def _step_emb(self, x_t, states):
        ...
        04elite Loop（P11）：每层按 loop_config[i] 重复执行，第 loop_idx 次循环使用
        states[i][loop_idx]（每次循环一份**独立** SSM 状态，与 forward/prefill 语义一致）。
        USE_LOOP=False 时 n_loops=1、_loop_res_scale=1.0、states[i] 长度为 1，退化为原实现。
        """
        for i, layer in enumerate(self.layers):
            n_loops = int(self.loop_config[i]) if i < len(self.loop_config) else 1
            _layer_in = x_t                               # 残差基准（循环间固定）
            _loop_res_scale = get_loop_residual_scale(n_loops, self.num_unique_layers)
            for loop_idx in range(n_loops):
                _cur = x_t
                if len(self.loop_embeds) > 0 and loop_idx > 0:
                    _cur = self.loop_embeds[i](_cur, loop_idx)
                y_t = layer.step(_cur, states[i][loop_idx])
                if self.use_residual:
                    y_t = y_t * _loop_res_scale + _layer_in
                y_t = self.layer_norms[i](y_t)
                if self.moe_enabled:
                    if USE_LOOP and LOOP_MOE_MODE == 'per_loop':
                        moe_out = self.moes[i][loop_idx].step(y_t)
                    else:
    ...
    def generate(self, input_ids: torch.Tensor, max_new_tokens: int = MAX_SEQ_LEN,
        ...
        if len(prompt_ids) == 0:
            # 空 prompt：用 BOS（缺省 0）起头，否则 prefill/step 会拿到空序列
            _bos = getattr(self, 'bos_token_id', None)
            prompt_ids = [_bos if _bos is not None else 0]
    ...
    try:
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                # ---- P1：按间隔清理显存缓存（只释放未使用缓存，不改数学）----
                if EMPTY_CACHE_INTERVAL > 0 and (step + 1) % EMPTY_CACHE_INTERVAL == 0:
                    if device.type == 'cuda':
...
# ------------------------------------------------------------
# P4：数据预处理 —— 超长对话切分（多轮切单轮 / 单轮超长丢弃）
# ------------------------------------------------------------
def p4_segment_dataset(input_path, output_path, tokenizer, threshold):
    """P4: 超长对话切分。

    规则：
      1. 解析每条对话的 conversations（多轮）
      2. 按 (user, assistant) 对切分
      3. 单对 token 长度 <= threshold → 保留为独立样本
      4. 单对 token 长度 >  threshold → 丢弃该对（不截断，不学半句）
      5. 一条多轮对话的所有对都超长 → 整条丢弃
      6. 不截断、不重训 tokenizer（只用现有词表切）
        ...
        for line in fin:
            ...
            for pair in pairs:
                ...
                for turn in pair:
                    ...
                    pair_tokens += len(ids) + 2      # role token + end token 开销
                # 与真实样本长度对齐：input_ids = tokens[:-1]（tokens = [BOS] + 各轮 + [EOS]），
                # 故真实长度 = Σ(len(ids)+2) + 1。不加这 1 会让边界样本多留一条。
                pair_tokens += 1

                if pair_tokens <= threshold:
                    record = {'conversations': list(pair)}
    ...
    log_and_print("=== P4 数据切分统计 ===")
    log_and_print(f"原行数: {stats['total_lines']}")
    log_and_print(f"保留样本数: {stats['kept_samples']}")
    log_and_print(f"保留的原行数: {stats['kept_lines']}")
    log_and_print(f"完全丢弃的行数: {stats['dropped_lines']}")
    log_and_print(f"丢弃的对话对数: {stats['dropped_pairs']}")
    log_and_print(f"输出文件: {output_path}")
    log_and_print(f"阈值: {threshold} tokens")
    return stats


def p4_segment_dataset_menu():
    """主菜单 11：数据预处理，切分超长对话。"""
    input_path = input("输入 jsonl 文件（默认 dialogues.jsonl）: ").strip() or 'dialogues.jsonl'
    if not os.path.exists(input_path):
        log_and_print(f"文件不存在: {input_path}")
        return

    default_out = input_path.replace('.jsonl', '_seg.jsonl')
    output_path = input("输出 jsonl 文件（默认 %s）: " % default_out).strip() or default_out
    if os.path.abspath(input_path) == os.path.abspath(output_path):
        log_and_print("输入输出不能是同一文件（避免覆盖）")
        return

    try:
        threshold_str = input(f"单样本 token 上限（默认 {P4_SEG_MAX_TOKENS}）: ").strip()
        threshold = int(threshold_str) if threshold_str else P4_SEG_MAX_TOKENS
    except ValueError:
        threshold = P4_SEG_MAX_TOKENS

    tokenizer = FastBPE(VOCAB_SIZE)
    if os.path.exists("tokenizer_incremental.bin"):
        if not tokenizer.load("tokenizer_incremental.bin"):
            log_and_print("加载 tokenizer 失败")
            return
    else:
        log_and_print("未找到 tokenizer_incremental.bin，无法切分")
        return

    p4_segment_dataset(input_path, output_path, tokenizer, threshold)
...
def main():
    ...
    while True:
        ...
        print("10. 退出")
        print("11. 数据预处理：超长对话切分（多轮切单轮 / 单轮超长丢弃）")
